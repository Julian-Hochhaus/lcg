...
class LCGApp:
    def __init__(self, root, camera_index, camera_list, tis):
        ...
        self.auto_exposure_value = 'Off'
        self.boolean_auto_gain = tk.BooleanVar()
        self.auto_gain_value = 'Off'
    ...
    def display_last_saved_image(self, frame):
        ...
        actual_height = self.camera.heigt
        print('test',actual_height)
    ...
    def capture_images_loop(self):
        ...
        self.save_directory=script_directory+'/../images'
        if self.start_energy_valid.cget('text') == 'Valid' and self.end_energy_valid.cget(
                'text') == 'Valid' and self.step_energy_valid.cget('text') == 'Valid':
            start = float(self.start_energy_entry.get())
            end = float(self.end_energy_entry.get())
    ...
    def capture_energy_image(self, energy, end_energy, step):
        if energy <= end_energy:
            ...
            self.camera.set_property('ExposureTime', self.approx_exposure[self.capture_step] * 10000)
            self.camera.set_property('Gain', self.approx_gain[self.capture_step])
            break_time = 1000 if self.camera.get_property('ExposureTime') // 10 < 1000 else self.camera.get_property('ExposureTime') // 10
            ...
            if status:
                readback_energy = self.leed_device.read_energy()[-1]

                self.result_label.config(text=f"Result: {readback_energy}")
                self.output_leed.config(text=f"Result: {readback_energy}")
                file_path = self.save_directory + f'/EN_{readback_energy}.png'
    ...
    def load_camera_settings(self):
        try:
            with open(script_directory + '/config.toml', 'r') as config_file:
                ...
                self.auto_gain_value = 'Off' if not config.get('CameraSettings', {}).get('gain_auto') else 'Continuous'
                self.boolean_auto_gain.set(True) if config.get('CameraSettings', {}).get(
                    'gain_auto') else self.boolean_auto_gain.set(False)
                self.auto_exposure_value = 'Off' if not config.get('CameraSettings', {}).get('exposure_auto') else 'Continuous'
    ...
    def add_calibration_datapoint(self):
        ...
        if energy is None:
            ...
            return
        gain = self.camera.get_property('Gain')
        exposure_time_in_s = self.camera.get_property('ExposureTime') // 10000
    ...
    def update_settings(self):
        try:
            self.update_settings_helper()
        except ValueError:
            traceback.print_exc()
    def update_settings_helper(self):
        selected_width, selected_height = self.resolutions[self.selected_resolution.get()]

        if self.camera.isRunning():
            self.camera.stop_pipeline()
        self.camera.set_property('GainAuto',self.auto_gain_value)
        frame_width = self.camera.width
        frame_height = self.camera.height
        frame_rate = self.camera.framerate
        self.frame_width_label.config(text=f"Frame Width: {frame_width}")
        self.frame_height_label.config(text=f"Frame Height: {frame_height}")
        self.frame_rate_label.config(text=f"Frame Rate: {frame_rate}")


        self.stream_status_label.config(text="Stream Status: Running", fg="green")
        self.stream_status_label.config(text="Stream Status: Stopped", fg="red")
        tis = TIS.TISCameraDevice()
        resolution=TIS.ResDesc(640,480,['1/1', '5/1', '15/1', '30/1', '60/1'])
        fps='1/1'
        self.camera=tis.open_device(self.available_cameras[self.camera_index], resolution, fps, TIS.SinkFormats.GRAY16_LE, False)

        self.camera.set_property('BlackLevel', self.brightness)
        self.camera.set_property('Gain', self.gain)
        self.camera.set_property('ExposureTime', self.exposure_time_absolute)
        self.auto_exposure_value = 'Off' if not self.boolean_auto_exposure.get() else 'Continuous'
        self.camera.set_property('ExposureAuto', self.auto_exposure_value)
    ...
    def update_settings_camera_ui(self):
        ...
        self.boolean_auto_gain.set(True) if self.auto_gain_value == 'Continuous' else self.boolean_auto_gain.set(False)
        self.boolean_auto_exposure.set(True) if self.auto_exposure_value == 'Continuous' else self.boolean_auto_exposure.set(False)


def main():
    try:
        cameras = TIS.available_cameras()
        tis, camera_index = TIS.choose_camera(cameras)
        with open(script_directory + '/config.toml', 'r') as config_file:
            config = toml.load(config_file)
